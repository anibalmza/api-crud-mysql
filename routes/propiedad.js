...
router.post('/', async (req, res) => {
   ...
   await pool.query('INSERT INTO propiedad set ?', [newPropiedad]);
   // res.json('INSERT ok');
   res.json({
      status: 'success',
      codigo: res.statusCode,
...
router.put('/:id', async (req, res) => {
   ...
   await pool.query('UPDATE propiedad set ? WHERE id = ?', [newPropiedad, id]);
   // res.json('UPDATE ok');
   res.json({
      status: 'success',
      codigo: res.statusCode,
...
router.delete('/:id', async (req, res) => {
   const { id } = req.params;
   await pool.query('DELETE FROM propiedad WHERE id = ?', [id]);
   // res.json('DELETE ok');
   res.json({
      status: 'success',
      codigo: res.statusCode,
